...
function updateLobbyPlayers(players) {
    ...
    lobbyPlayers.innerHTML = Object.values(players).map(player => `
...
function getStanding(players, player) {
    ...
    const uniqueHealths = [...new Set(Object.values(players).map(p => p.health))].sort((a, b) => b - a);
...
function updateBoard(questions) {
    ...
    board.innerHTML = questions.map((q, index) => `
